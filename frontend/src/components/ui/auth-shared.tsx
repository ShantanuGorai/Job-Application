...
export function GlassInput({ icon, inputRef, type, placeholder, value, onChange, onKeyDown, hasValue, showToggle, toggleShow, isPasswordVisible, showSubmit, submitIcon, onSubmit, submitAriaLabel, paddingRight }: {
    ...
    <div className="glass-input-wrap w-full">
      <div className="glass-input">
        ...
        <div className="relative z-10 flex-shrink-0 flex items-center justify-center w-10 pl-2">
          {showToggle ? (
            <button type="button" aria-label="Toggle password visibility" onClick={toggleShow} className="text-foreground/80 hover:text-foreground transition-colors p-2 rounded-full">
              {isPasswordVisible ? <EyeIconHidden /> : <EyeIconVisible />}
            </button>
          ) : (
            icon
          )}
        ...
        />
        <div className={cn("relative z-10 flex-shrink-0 overflow-hidden transition-all duration-300 ease-in-out", showSubmit ? "w-10 pr-1" : "w-0")}>
          <GlassButton type="button" onClick={onSubmit} size="icon" aria-label={submitAriaLabel} contentClassName="text-foreground/80 hover:text-foreground">
            {submitIcon}
          </GlassButton>
        </div>
      </div>
...
export function AuthShell({ brandName, logo, children, modalStatus }: {
  brandName: string;
...
export function AuthModal({ modalStatus, modalErrorMessage, modalSteps, onClose, successContent }: {
  modalStatus: 'closed' | 'loading' | 'error' | 'success';
  modalErrorMessage: string;
  modalSteps: { message: string; icon: React.ReactNode }[];
  onClose: () => void;
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative bg-card/80 border-4 border-border rounded-2xl p-8 w-full max-w-sm flex flex-col items-center gap-4 mx-2">
            {(modalStatus === 'error' || modalStatus === 'success') && (
              <button onClick={onClose} className="absolute top-2 right-2 p-1 text-muted-foreground hover:text-foreground transition-colors">
            ...
            {modalStatus === 'error' && (
              <>
                <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-12 h-12 text-destructive"><circle cx="12" cy="12" r="10" /><line x1="12" x2="12" y1="8" y2="12" /><line x1="12" x2="12.01" y1="16" y2="16" /></svg>
                <p className="text-lg font-medium text-foreground">{modalErrorMessage}</p>
                <GlassButton onClick={onClose} size="sm" className="mt-4">Try Again</GlassButton>
              </>
            )}
            {modalStatus === 'loading' && (
            ...
            {modalStatus === 'success' && successContent}
          </motion.div>
        </motion.div>
      )}
    </AnimatePresence>
